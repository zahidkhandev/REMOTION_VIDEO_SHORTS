...
6. **Visuals are CUSTOM** (You MUST prioritize `lucide_icon` and use `path` for non-icon shapes)
...
3.  **VISUALS MUST MATCH NARRATION (PRIORITIZE ICONS):**

    - Read the `narration` text for the scene. You **MUST** generate visuals that represent the words.
    - **Example:** If narration mentions "rocket," you MUST use `lucide_icon` with `data: "Rocket,80"`.
    - **Example:** If narration mentions "data," you MUST use `lucide_icon` with `data: "Database,80"`.
    - **Example:** If narration mentions a graph, you MUST draw it with a `path`.
...
### `lucide_icon` (NEW - PRIORITY 1)

- **Description:** Use for high-quality, professional icons from the Lucide library. **PRIORITIZE THIS** over `path` for common icons (e.g., 'Rocket', 'Brain', 'Database', 'User', 'Settings', 'FileText', 'BarChart', 'Server', 'Cpu', 'GitBranch').
- **`data`:** Comma-separated `IconName,size` (e.g., `'Rocket,80'`). `IconName` is the string name from Lucide React. `size` is the pixel dimension (width/height).
...
### `path` (PRIORITY 2 - FOR CUSTOM SHAPES)

- **Description:** Use for **math curves, custom shapes, and illustrations** that are _not_ available as a `lucide_icon`.
- **CRITICAL:** Do NOT use this to draw simple icons like rockets, brains, or databases. Use `lucide_icon` for those.
...
type: "string"
enum: ["lucide\_icon", "path", "circle", "rect", "line", "text", "equation"]
data:
...
type: "number"
required: ["type", "data", "x", "y", "color", "stroke\_width", "animation", "delay"]
required: ["background\_color", "svg\_elements"]
required: ["title", "narration", "visual", "duration\_in\_seconds", "sound\_effect", "visuals"]
required: ["script\_id", "title", "total\_duration\_seconds", "scenes"]
...
- "**CRITICAL (ICONS):** You **MUST** use the `lucide_icon` type for all common icons (e.g., 'Rocket', 'Brain', 'Database', 'User', 'Settings', 'FileText', 'BarChart', 'Server', 'Cpu', 'GitBranch'). Only use `path` for truly custom shapes or graphs that do not exist in the Lucide library."
...
- "**CRITICAL (VISUALS):** You MUST generate `lucide_icon` and `line` connectors. A video with only text and circles is a failure. You are a visual artist; create custom visuals."
  `
